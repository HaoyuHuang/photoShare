...
public class BitmapRefCache {
	static private BitmapRefCache cache;
	/** 用于Chche内容的存储 */
	 ...
	 * 取得缓存器实例
	 */
	public static BitmapRefCache getInstance() {
		if (cache == null) {
	...
	public Bitmap getBitmap(byte[] bytes, String name){
		Bitmap bitmapImage = null;
		// 缓存中是否有该Bitmap实例的软引用，如果有，从软引用中取得。
		if (bitmapRefs.containsKey(name)) {
			BtimapRef ref = (BtimapRef) bitmapRefs.get(name);
			bitmapImage = (Bitmap) ref.get();
		}
		// 如果没有软引用，或者从软引用中得到的实例是null，重新构建一个实例，
		// 并保存对这个新建实例的软引用
		if (bitmapImage == null) {
			...
			this.addCacheBitmap(bitmapImage, name);
		}
		return bitmapImage;
	}
